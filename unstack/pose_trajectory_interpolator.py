"""
Code adapted from the universal manipulation interface library:
https://github.com/real-stanford/universal_manipulation_interface/blob/main/umi/common/pose_trajectory_interpolator.py
"""
...
def rotation_distance(a: st.Rotation, b: st.Rotation) -> float:
    return float((b * a.inv()).magnitude())
    ...
    def schedule_waypoint(
        ...
        if last_waypoint_time is not None:
            if time <= last_waypoint_time:
                ...
                assert last_waypoint_time is not None and curr_time is not None
