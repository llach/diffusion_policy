...
class PolicyNode(Node):
    def __init__(self):
        ...
        self.delta_pose_pub = self.create_publisher(PoseStamped, 'desired_pose_delta', 10)
    ...
    def get_start_pose_thread(self):
        ...
        while attempts < self.max_start_pose_attempts:
            try:
                ...
                eef_pos, eef_rot_axis_angle = self.get_robot_eef_pose()
                ...
                # TODO dummy values. get this via callback or infer from joint states
    ...
    def update_pose_buffers(self):
        eef_pos, eef_rot_axis_angle = self.get_robot_eef_pose()

        self.robot0_eef_pos_buffer.append(eef_pos)
        current_rot_mat = st.Rotation.from_rotvec(eef_rot_axis_angle).as_matrix()
        rot6d = mat_to_rot6d(current_rot_mat)
        ...
        relative_rot_mat = relative_rot.as_matrix()
        rel_rot6d = mat_to_rot6d(relative_rot_mat)
    ...
    def process_policy_output(self, raw_action_t):
        """
        Processes the raw action output from the policy to determine the desired EEF pose
        and computes the delta between the desired pose and the current pose for debugging.

        Args:
            raw_action_t (numpy.ndarray): The raw action output from the policy. 
                                        Expected shape: [10], containing [pos (3), rot6d (6), gripper_open (1)]

        Returns:
            tuple:
                - action_pred (dict): Contains the desired 'position', 'rotation', and 'gripper_open' state.
                - debug_info (dict): Contains the delta changes in position and rotation, and gripper state information.
        """
        # Extract position, rotation (6D), and gripper open value from raw_action_t
        pos = raw_action_t[:3]
        ...
        if action_repr.startswith('abs'):
            ...
            desired_pos = start_pos + pos
            desired_rotvec = start_rotvec + rotvec
            ...
            if len(self.robot0_eef_pos_buffer) == 0 or len(self.robot0_eef_rot_axis_angle_buffer) == 0:
                ...
                desired_pos = current_pos + pos
                desired_rotvec = current_rotvec + rotvec
        ...
        # Fetch current pose to compute delta for debug info
        current_pos, current_rotvec = self.get_robot_eef_pose()
        if current_pos is None or current_rotvec is None:
            # If current pose unavailable, use desired pose as delta
            delta_pos = desired_pos
            delta_rotvec = desired_rotvec
            self.get_logger().warn("Current pose unavailable. Using desired pose as delta.")
        else:
            # Compute delta between desired pose and current pose
            delta_pos = desired_pos - current_pos

            # Compute rotation difference using quaternion multiplication to find the relative rotation
            current_rot = st.Rotation.from_rotvec(current_rotvec)
            desired_rot = st.Rotation.from_rotvec(desired_rotvec)
            rot_diff = desired_rot * current_rot.inv()
            delta_rotvec = rot_diff.as_rotvec()

        # Prepare action prediction dictionary
        action_pred = {
            'position': desired_pos,
            'rotation': desired_rotvec,
            'gripper_open': gripper_open_raw
        }

        # Prepare debug information dictionary
        debug_info = {
            'delta_pos_cm': delta_pos * 100.0,  # Convert position delta to centimeters
            'delta_rot_rpy': st.Rotation.from_rotvec(delta_rotvec).as_euler('xyz', degrees=True),  # Convert rotation delta to Roll-Pitch-Yaw in degrees
    ...
    def publish_outputs(self, action_pred, debug_info):
        ...
        pose_msg.pose.position.x = float(debug_info['delta_pos_cm'][0])
        pose_msg.pose.position.y = float(debug_info['delta_pos_cm'][1])
        pose_msg.pose.position.z = float(debug_info['delta_pos_cm'][2])
        # Convert rotation vector to quaternion
        rotvec = action_pred['rotation']
        quat = st.Rotation.from_rotvec(rotvec).as_quat()
        pose_msg.pose.orientation.x = debug_info['delta_rot_rpy'][0]
        pose_msg.pose.orientation.y = debug_info['delta_rot_rpy'][1]
        pose_msg.pose.orientation.z = debug_info['delta_rot_rpy'][2]
        pose_msg.pose.orientation.w = .0
        self.delta_pose_pub.publish(pose_msg)

        # Publish the desired pose delta
        delta_pose_msg = PoseStamped()
        delta_pose_msg.header.stamp = self.get_clock().now().to_msg()
        delta_pose_msg.header.frame_id = self.target_frame  # Adjust the frame as needed  
        delta_pose_msg.pose.position.x = float(action_pred['position'][0])
        delta_pose_msg.pose.position.y = float(action_pred['position'][1])
        delta_pose_msg.pose.position.z = float(action_pred['position'][2])
        # Convert rotation vector to quaternion
        rotvec = action_pred['rotation']
        quat = st.Rotation.from_rotvec(rotvec).as_quat()
        delta_pose_msg.pose.orientation.x = quat[0]
        delta_pose_msg.pose.orientation.y = quat[1]
        delta_pose_msg.pose.orientation.z = quat[2]
        delta_pose_msg.pose.orientation.w = quat[3]
        self.pose_pub.publish(delta_pose_msg)
    ...
    def create_debug_image(self, debug_info):
        ...
        with self.lock:
            if len(self.camera_buffer) > 0:
                ...
                # Convert RGB to BGR
                latest_camera_image = cv2.cvtColor(latest_camera_image, cv2.COLOR_RGB2BGR)
            else:
